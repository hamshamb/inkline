...
export function TopBar({ onOpenSidebarDrawer, onOpenQuickOpen, focusMode, onExitFocusMode }: TopBarProps) {
  ...
  return (
    <div className="flex h-11 shrink-0 items-center gap-3 border-b border-[var(--color-border)] bg-[var(--color-chrome)] px-3">
      ...
      <span className="text-[13px] font-medium tracking-tight text-[var(--color-secondary)]">{product.name}</span>
      <button
        type="button"
        onClick={onOpenQuickOpen}
        className="mx-auto flex w-full max-w-xs items-center justify-center gap-2 rounded-md border border-[var(--color-border)] px-3 py-1 text-xs text-[var(--color-muted)] hover:border-[var(--color-border-strong)] hover:bg-[var(--color-hover)]"
      >
